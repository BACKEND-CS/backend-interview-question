...
<details>
 ...
 <summary><h2> 📳Spring </h2> </summary>
  ...
  <details>
  <summary> Spring DI/IoC는 대해서 설명해주세요.</summary>
      
      DI란 객체를 직접 생성하는 게 아니라 IoC컨테이너에 등록된 빈 설정 정보를 바탕으로 외부에서 의존성을 주입해주는 것을 말합니다.
      DI를 통해 모듈 간의 결합도가 낮아지고 유연성이 높아지는 장점이 있습니다.
      의존성 주입 종류로는 Field Injection, Setter Injection, Constructor Injection 방법이 있습니다.
      
      IoC는 제어의 역전으로 객체 제어를 직접 제어하는 것이 아니라 외부에서 프레임워크가 관리하는 것을 말합니다.
      
  </details>
  
  <details>
  <summary> Spring Bean의 정의와 등록방법에 대해서 설명해주세요.</summary>
       
       스프링 IoC 컨테이너가 관리하는 자바 객체를 의미합니다. 
       등록방법은,
       1. 컴포넌트 스캔과 자동 의존관계 설정
          클래스 선언부 위에 @Component 어노테이션을 사용하면, 해당 어노테이션으로 등록된 클래스들은
          스프링 컨테이너에 의해 자동으로 생성되어 스프링 빈으로 등록됩니다.
       2. 자바 코드로 직접 스프링 빈 등록
          설정 클래스를 만들고 @Configuration 어노테이션을 클래스 선언부에 추가한 뒤,
          @Bean을 메서드 위에 붙여주면 해당 타입의 빈 객체가 등록됩니다.
  ...
  <summary> 스프링 Bean의 생성 과정을 설명해주세요.</summary>
        ...
        스프링 컨테이너에 의해 관리되며,
        객체 생성 -> 의존성 주입 -> 초기화 -> 사용 -> 소멸의 생명주기를 가지고 있습니다.
        
        스프링 컨테이너가 초기화될 때 먼저 빈 객체를 설정에 맞춰 생성하며 의존 관계를 설정한 뒤 
        해당 프로세스가 완료되면 빈 객체가 지정한 메소드를 호출해서 초기화를 진행합니다.
        
        어노테이션을 이용한 빈 초기화 방법엔 @PostConstruct를, 빈 소멸에는 @PreDestroy를 사용합니다.
